...
test("UUID API contracts cover all record types and concurrent creation", async () => {
  ...
  await api("PUT", `/api/jobs/${job.id}`, { status: "completed" });
  await api("PUT", `/api/invoices/${invoice.id}`, { status: "paid" });
  await api("PUT", `/api/jobs/${jobs[2].id}`, { status: "cancelled", scheduled_date: "2027-01-01" });
  const customerContext = (await api("GET", `/api/jobs/${jobs[1].id}`)).job;
  assert.equal(customerContext.customer_job_count, 4);
  assert.equal(customerContext.customer_last_service_id, job.id, "scheduled and cancelled work must not become the last service");
  assert.equal(customerContext.customer_last_service_identifier, job.identifier);
  assert.equal(customerContext.customer_last_service_date, "2026-09-15");
  assert.equal(customerContext.customer_lifetime_revenue, 145);
  await api("PUT", `/api/jobs/${jobs[1].id}`, { status: "completed", scheduled_date: "2026-10-15" });
  assert.equal((await api("GET", `/api/jobs/${jobs[1].id}`)).job.customer_last_service_id, job.id, "a completed job must not identify itself as prior service");
  const firstVisitCustomer = await api("POST", "/api/customers", { name: "First visit" }, 201);
  const firstVisitJob = await api("POST", "/api/jobs", { customer_id: firstVisitCustomer.id, scheduled_date: "2026-10-20" }, 201);
  const firstVisitContext = (await api("GET", `/api/jobs/${firstVisitJob.id}`)).job;
  assert.equal(firstVisitContext.customer_job_count, 1);
  assert.equal(firstVisitContext.customer_last_service_id, null);
  assert.equal(firstVisitContext.customer_lifetime_revenue, 0);
