-- Migration number: 0001 	 2026-08-17T17:43:05.980Z
--
-- Baseline migration: this is a verbatim snapshot of src/server/schema.sql as it stood
-- before the FSM upgrade work began. schema.sql itself is now historical/reference-only;
-- all further schema changes are new numbered files in this directory, applied via
-- `wrangler d1 migrations apply open-fieldservice-db --local` (see package.json "dev").
-- Every statement below is idempotent (CREATE ... IF NOT EXISTS / INSERT OR IGNORE) on
-- purpose, so this migration is a safe no-op against databases that already have these
-- tables from the old direct `wrangler d1 execute --file=schema.sql` bootstrap.
